...
void ServerWindow::receive_from_client(QTcpSocket* client, NetworkData data) {
    ...
    case OPCODE::READY_OP:
        if(!onGame)
        {
            ...
            HisName=data.data1;
            GameRequest(data.data1,data.data2,data.data.3);
        }
        break;
...
void ServerWindow::send_to_client(QTcpSocket* client, NetworkData data){
    ...
    case OPCODE::READY_OP:
        ...
        {
            HisName=data.data1;
            SendRequest(data.data1,data.data2,data.data.3);
        }
        break;
    ...
    this->server->send(client,data);
}

//REJECT_OP   收到两个客户端执同色棋的READY_OP时触发
void ServerWindow::reject_client(QTcpSocket* client,NetworkData data)
{
    QString rejectreason;
    this->server->send(client,NetworkData(OPCODE::REJECT_OP,MyName,rejectreason));
    ui->ShowClient1->setText(data.data1,data.data2);
    }

}

//拒绝这个申请
void ServerWindow::RejectOne(QString rejectreason,QTcpSocket * soc)
{
    client=soc;
    Requestcnt--;
    if(Requestcnt == 0) rt->hide();
    RejectGame(rejectreason);
}
