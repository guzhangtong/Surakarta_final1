...
//客户端发送对局请求 发送READY_OP（1）
void ClientWindow::sendRequest()
{
    if(!onGame)
    {

        if(ui->ColorButton->isChecked()) color="BLACK";
        else color="WHITE";
        this->socket->send(NetworkData(OPCODE::READY_OP,MyName,color,room));
        ui->StatusLabel->setText("Waiting for Reply");
    }
    else if(onGame)
    {
        ui->StatusLabel->setText("On Game!No Request");
    }
}
